...
function randomGlyphLine(length: number) {
  let out = '';
...
function TerminalBox() {
  const [lines, setLines] = useState<string[]>(() =>
    Array.from({ length: LINE_COUNT }, () => randomGlyphLine(10 + Math.floor(Math.random() * 24)))
  );

  useEffect(() => {
    const interval = setInterval(() => {
      setLines(prev =>
        prev.map(() => randomGlyphLine(10 + Math.floor(Math.random() * 24)))
      );
    }, 110);
