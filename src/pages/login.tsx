...
function BackgroundTerminal() {
  const lines = useMemo(() => [...TERMINAL_LOG, ...TERMINAL_LOG, ...TERMINAL_LOG], []);

  return (
    <div
      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[min(94vw,760px)] h-[min(80vh,560px)] rounded-2xl border border-primary/15 bg-black/40 overflow-hidden pointer-events-none select-none"
      aria-hidden="true"
    >
      {/* Barra de título */}
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-primary/10 bg-white/5">
        ...
        <span className="ml-3 font-mono text-[11px] tracking-wider text-primary/40">
          system — bash
        </span>
      </div>

      {/* Contenido con scroll infinito */}
      <div className="relative h-full overflow-hidden">
        <div className="absolute inset-x-0 top-0 animate-terminal-scroll px-6 py-4">
          {lines.map((line, i) => (
            <div key={i} className="font-mono text-xs sm:text-sm leading-8 whitespace-nowrap text-primary/20">
              {line}
            </div>
          ))}
        </div>
      ...
      {/* Vignette para que se funda con el fondo */}
      <div className="absolute inset-0 bg-[radial-gradient(circle,transparent_30%,black_95%)]" />
    </div>
  );
}
...
export function LoginScreen({ login }: { login: (pwd: string) => Promise<boolean> }) {
  ...
  return (
    <div className="min-h-[100dvh] w-full flex flex-col items-center justify-center p-4 relative overflow-hidden">
      ...
      {/* Panel de terminal grande, decorativo, detrás del formulario */}
      <BackgroundTerminal />
      ...
      <div className="w-full max-w-[420px] flex flex-col items-center relative z-10">
        ...
        <h1 className="text-2xl font-bold text-white mb-2 tracking-tight">RANDY ALEJANDRO</ h1>
